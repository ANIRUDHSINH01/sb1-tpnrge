import React from 'react';
import { Bot, Server, Zap } from 'lucide-react';

export function Hero() {
  return (
    <div className="relative bg-gray-900 overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div className="relative z-10 pb-8 sm:pb-16 md:pb-20 lg:max-w-2xl lg:w-full lg:pb-28 xl:pb-32">
          <main className="mt-10 mx-auto max-w-7xl px-4 sm:mt-12 sm:px-6 md:mt-16 lg:mt-20 lg:px-8 xl:mt-28">
            <div className="sm:text-center lg:text-left">
              <h1 className="text-4xl tracking-tight font-extrabold text-white sm:text-5xl md:text-6xl">
                <span className="block">Premium Hosting for</span>
                <span className="block text-indigo-500">Discord Bots & Minecraft</span>
              </h1>
              <p className="mt-3 text-base text-gray-300 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">
                Experience seamless integration of Discord bot hosting and Minecraft servers. 
                Powered by cutting-edge infrastructure for optimal performance and reliability.
              </p>
              <div className="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
                <div className="rounded-md shadow">
                  <a href="#pricing" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10">
                    Get Started
                  </a>
                </div>
                <div className="mt-3 sm:mt-0 sm:ml-3">
                  <a href="#features" className="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 md:py-4 md:text-lg md:px-10">
                    Learn More
                  </a>
                </div>
              </div>
            </div>
          </main>
        </div>
      </div>
      <div className="lg:absolute lg:inset-y-0 lg:right-0 lg:w-1/2">
        <div className="h-56 w-full bg-gray-800 sm:h-72 md:h-96 lg:w-full lg:h-full flex items-center justify-center">
          <div className="grid grid-cols-2 gap-8 p-8">
            <div className="bg-gray-900 p-6 rounded-lg">
              <Bot className="h-12 w-12 text-indigo-500 mb-4" />
              <h3 className="text-white font-semibold">Discord Bots</h3>
              <p className="text-gray-400 text-sm mt-2">24/7 Uptime</p>
            </div>
            <div className="bg-gray-900 p-6 rounded-lg">
              <Server className="h-12 w-12 text-indigo-500 mb-4" />
              <h3 className="text-white font-semibold">Minecraft Servers</h3>
              <p className="text-gray-400 text-sm mt-2">High Performance</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}